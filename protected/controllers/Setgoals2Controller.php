...
class Setgoals2Controller extends Controller
{
	public $the_id = "";
...
if ($emp_data['0']['reporting_1_change'] != '' && strtotime($emp_data['0']['reporting_1_effective_date'])<=strtotime(date('Y-m-d'))) {
		...
		$this->render('//site/set_goal_auto_savedemo',array('model'=>$model,'kra_list'=>$kra_types,'kpi_data_edit'=>$kpi_data,'KRA_category_auto'=>$KRA_category,'kpi_data'=>$kpi_data_saved,'kpi_data1'=>$kpi_data,'emp_data'=>$emp_data,'prg_cnt'=>$prg_cnt,'goal_sub_check_flag'=>$goal_sub_check_flag,'kpi_data2'=>$kpi_data2,'edit_flag'=>""));
...
$notification_data->notification_type = 'KPI Deletion';
   ...
   $employee_email = Yii::app()->user->getState("employee_email");
	...
	function actionkpi_edit($KPI_id = NULL)
	{
		...
		$this->render('//site/session_check_view');
		$this->render('//site/header_view_layout');
		$this->render('//site/set_goal_auto_savedemo',array('model'=>$model,'kra_list'=>$kra,'kpi_data'=>$kpi_data,'kpi_data_edit'=>$kpi_data_edit,'edit_flag'=>'1','KRA_category_auto'=>$KRA_category));
		$this->render('//site/footer_view_layout');
	}
...
$where = 'where goal_set_year = :goal_set_year';
	...
	//print_r($program_data_result);die();	
		...
		$this->render('//site/edit_goal_sheet12',array('kpi_auto_data'=>$kpi_data , 'model'=>$model,'kra_list'=>$kra,'kpi_data'=>$kpi_data,'program_data_result'=>$program_data_result,'kpi_data_edit'=>$kpi_data_edit,'apr_chk_flag'=>'1','KRA_category_auto'=>$KRA_category,'emp_data'=>$emp_data,'edit_flag_chk'=>1,'prg_cnt'=>$prg_cnt,'show_idp'=>$show_idp));
		$this->render('//site/footer_view_layout');
	}
