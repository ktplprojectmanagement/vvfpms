...
class Download_goalsheet1Controller extends Controller
{
	public function actionIndex()
	{
		...
		if (isset($settings_data['0']['setting_type']) && count($settings_data)>0) {
			...
			$year1=$settings_data['0']['setting_type'];
					...
					$kpi_data=$model1->get_disinct_kra_appr($year1);
			...
			if (isset($kpi_data) && count($kpi_data)>0 && isset($settings_data['0']['setting_type'])) {
				...
				for ($i=0; $i < count($kpi_data); $i++) { 
					...
					$kpi_data_edit=$model1->get_disinct_kra_appr($year1);
					//print_r($kpi_data_edit);die();

					$where = 'where Employee_id = :Employee_id';
					$list = array('Employee_id');
					$data = array($kpi_data_edit[$i]['Employee_id']);
...
if (Yii::app()->user->getState("Employee_id")!='') 
		...
		$this->render('//site/DownloadGoalsheet1',array('kpi_data_edit'=>$kpi_data_edit,'employee_list'=>$emp_data));        
		}
		else{
		$model=new LoginForm;
