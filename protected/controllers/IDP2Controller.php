...
class IDP2Controller extends Controller
{
...
public function actionq1_subordinate_idp()
	{
			if (isset($_POST['emp_id'])) {
			$emp_id = Yii::app()->user->setState('emp_id_4',$_POST['emp_id']);
			}
...
//echo $_POST['number_of_meetings'];die();
		if (count($IDP_data)>0) {
				$detail = array(
				// 'Employee_id' => $_POST['emp_code'],
				...
				'topic_to_be_diss' => $_POST['topic_diss1'],
...
$detail = array(
	...
	function actionmid_save_data()
	{
		...
		$IDP_data = $model->get_idp_data($where,$data,$list);
		if (count($IDP_data)>0) {
		   // echo count($IDP_data);die();
			$detail = array(
			//	'Employee_id' => $_POST['emp_code'],
				'q1_prgrm_cmd' => $_POST['prgrm_cmd'],
				'q1_mgr_trn_prog_stat' => $_POST['program_status'],
				'q1_extra_prgrm_cmd' => $_POST['extra_prgrm_cmd'],
				'q1_extra_program_status' => $_POST['extra_program_status'],
				'q1_rel_program_review' => $_POST['rel_program_review'],
				'q1_rel_prog_cmt_mgr' => $_POST['rel_program_review_status'],
...
//print_r(Yii::app()->user->getState('financial_year_check'));die();
			$update = Yii::app()->db->createCommand()->update('IDP',$detail,'Employee_id=:Employee_id and goal_set_year=:goal_set_year',array(':Employee_id'=>$_POST['emp_code'],':goal_set_year'=>'2017-2018'));
//print_r($detail);die();
			if($update>0)
		  	{
		  		print_r("Successfully Saved");die();
