...
class KpiListForm extends CActiveRecord
{
	...
	function get_seach($chk_value1,$chk_value)
	{		
		$connection=Yii::app()->db;
		$sql = "select * from `kpi_list`".' '.'where department = :department and kpi_description like :kpi_description';
		//print_r($sql);die();
		$command=$connection->createCommand($sql);		
		$command->bindValue(":".'department',$chk_value1);
	...
	{
		$connection=Yii::app()->db;
		$sql = "select * from `kpi_list`  where kpi_description != '--Select--'";
		$command=$connection->createCommand($sql);
		$rows=$command->queryAll();		
		return $rows;
	}
	...
	{		
		$connection=Yii::app()->db;
		$sql = "select * from `kpi_list`".' '.$where;
		//print_r($sql);die();
		$command=$connection->createCommand($sql);
