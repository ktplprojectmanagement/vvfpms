...
class NotificationsForm extends CActiveRecord
{
	...
	function get_submitted_notificationdata()
	{
		$connection=Yii::app()->db;
		$sql = "select * from notifications where notification_type LIKE '%submitted%' OR notification_type LIKE '%done%' ORDER BY id DESC";
		$command=$connection->createCommand($sql);
		$rows=$command->queryAll();
		return $rows;
	}

	function get_pending_notificationdata()
	{
		$connection=Yii::app()->db;
		$sql = "select * from notifications where notification_type LIKE '%pending%' ORDER BY id DESC";
		$command=$connection->createCommand($sql);
		$rows=$command->queryAll();
		return $rows;
	}

	function get_approve_notificationdata()
	{
		$connection=Yii::app()->db;
		$sql = "select * from notifications where notification_type LIKE '%approved%' ORDER BY id DESC";
		$command=$connection->createCommand($sql);
		$rows=$command->queryAll();
		return $rows;
	}
