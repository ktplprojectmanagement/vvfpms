...
class EmployeeForm extends CActiveRecord
{
	...
	{
		return array(
			array('Employee_id, Employee_atd_code, Emp_fname,Emp_lname,DOB,joining_date,mobile_number,PAN_number,Email_id,Present_address,Permanent_address,Department,Designation,Gender,Cadre,Nationality,Employee_status,Reporting_officer1_id,cluster_name,cluster_appraiser','required'),
			array('Emp_fname','match','pattern'=>'/^([A-Za-z0-9]{1,})$/', 'message'=>'Please Enter valid first name'),
			array('Emp_lname','match','pattern'=>'/^([A-Za-z0-9]{1,})$/', 'message'=>'Please Enter valid last name'),
	...
	function getdata()
	{
		...
		$sql = "select * from Employee where company_location='".Yii::app()->user->getState('admin_location')."'";
	...
	function get_appraiser_list()
	{
		if (Yii::app()->user->getState('role_id') == '4') {
			...
			$connection=Yii::app()->db;
			$sql = "select distinct `Reporting_officer1_id` from `Employee` where company_location='".Yii::app()->user->getState('admin_location')."'";
			$command=$connection->createCommand($sql);
			$rows=$command->queryAll();		
			return $rows;
		}
