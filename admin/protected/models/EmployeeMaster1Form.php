 ...
 */
class EmployeeMaster1Form extends CActiveRecord
{
	/**
	...
	function getdata()
	{
		...
		$sql = "select * from Employee_master1 ";
	...
	{		
		$connection=Yii::app()->db;
		$sql = "select * from `Employee_master1` ".' '.$where;
    ...
    {
     $connection=Yii::app()->db;
     $sql = 'select * from `Employee_master1`  WHERE company_location IN ("Kolkata", "Corporate", "chennai", "kutch-II", "palanpur", "raipur")';
	...
	{
		$connection=Yii::app()->db;
		$sql = "select distinct `Department` from `Employee_master1`  where Department != ''";
		$command=$connection->createCommand($sql);
		$rows=$command->queryAll();		
		return $rows;
	}
	...
	{
		$connection=Yii::app()->db;
		$sql = "select distinct `Reporting_1_for_time_n_attendance` from `Employee_master1` ";
		$command=$connection->createCommand($sql);
		$rows=$command->queryAll();		
		return $rows;
	}
	...
	{
		$connection=Yii::app()->db;
		$sql = "select distinct `Reporting_1_for_appraisal` from `Employee_master1` ";
		$command=$connection->createCommand($sql);
		$rows=$command->queryAll();		
		return $rows;
	}
	...
	{
		$connection=Yii::app()->db;
		$sql = "select distinct `Reporting_officer2_id` from `Employee_master1` ";
		$command=$connection->createCommand($sql);
		$rows=$command->queryAll();		
		return $rows;
	}
	...
	{
		$connection=Yii::app()->db;
		$sql = "select distinct `Manager_manager` from `Employee_master1` ";
		$command=$connection->createCommand($sql);
		$rows=$command->queryAll();		
		return $rows;
	}
	...
	{
		$connection=Yii::app()->db;
		$sql = "select distinct `cluster_appraiser` from `Employee_master1` ";
		$command=$connection->createCommand($sql);
		$rows=$command->queryAll();		
		return $rows;
	}
	...
	{
		$connection=Yii::app()->db;
		$sql = "select distinct `Designation` from `Employee_master1` where Designation != ''";
		$command=$connection->createCommand($sql);
		$rows=$command->queryAll();		
		return $rows;
	}
	...
	{
		$connection=Yii::app()->db;
		$sql = "select distinct `BU` from `Employee_master1` where BU != ''";
		$command=$connection->createCommand($sql);
		$rows=$command->queryAll();		
		return $rows;
	}
